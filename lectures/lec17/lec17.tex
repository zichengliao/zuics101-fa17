...
\begin{frame}
  ...
  \myitem Homework \#7 will be out later this week. \pause
  \myitem  Midterm \#2 is Next Thursday, Nov. 30 (in class)��.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \myitem  What are some sources of genuine randomness? \pause
  ...
  $$  \pause
  \begin{enumerate}
  \myitem  These are derived from the same rule ($\pi$/4)---but only one seems ``random'' to us.
  \end{enumerate}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \myitem It is designed to draw random numbers following some distribution.
...
np.random.randn() + 1.0  # mean 1.0
  ...
  %\pause
  \begin{Verbatim}
(np.random.randn()) * 4  # std 4.0
  \end{Verbatim}
  \mysubitem  %(Variance is the square of standard deviation.)
  
  \myitem how to create a normal distribution of mean -10 and standard deviation 0.2?
  \end{enumerate}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{enumerate}
  \myitem  \texttt{choice} randomly samples from a one dimensional \emph{array} or \emph{list} (rather than the first dimension of the array).
  \end{enumerate}
  \begin{Verbatim}
x = ['red','orange','yellow','green','blue']
np.random.choice(x) # samples a color from x
  \end{Verbatim}
\end{frame}
...
\begin{frame}[fragile]
  ...
  Which of the following will \emph{not} reproduce the behavior of a six-sided die?

  \begin{enumerate}[label=\Alph*]
  \item
  \begin{Verbatim}
d = np.random.randn( 6 ) + 1
  \end{Verbatim}
  \item
  \begin{Verbatim}
x = np.arange( 1,7 )
d = np.random.choice( x )
  \end{Verbatim}
  \item
  \begin{Verbatim}
d = np.random.randint( 6 )+1
  \end{Verbatim}
  \item
  \begin{Verbatim}
...
\begin{frame}[fragile]
  ...
  \myitem    Example:  Sentence Generator
  \end{enumerate}
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]
  \frametitle{Sentence generator}
...
# note names and verbs have a slightly different structure
# than adj and nouns
  \end{Verbatim}
\end{frame}
